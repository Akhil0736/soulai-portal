...
import com.soulai.portal.input.SoulAIKeyboardIME
import com.soulai.portal.core.JsonBuilders
import com.soulai.portal.core.StateRepository
import com.soulai.portal.service.GestureController
...
import com.soulai.portal.service.ScreenCaptureService
import com.soulai.portal.streaming.WebRtcManager
...
class ApiHandler(
    ...
    private val getKeyboardIME: () -> SoulAIKeyboardIME?,
    ...
    fun keyboardKey(keyCode: Int): ApiResponse {
        ...
        val ime = getKeyboardIME()
            ?: return ApiResponse.Error(
                "SoulAIKeyboardIME not active or available. Use keyboard/input for text, or supported keys like back/home/recents/enter/delete.",
            )
    ...
    fun startApp(packageName: String, activityName: String? = null): ApiResponse {
        val service = SoulAIAccessibilityService.getInstance()
    ...
    fun installApp(
        ...
        return try {
            if (!context.packageManager.canRequestPackageInstalls()) {
                ...
                return ApiResponse.Error("Install permission denied. Please enable 'Install unknown apps' for Soul AI Portal in Settings.")
            }
            ...
            session.use {
                ...
                val action = "com.soulai.portal.INSTALL_COMPLETE_${sessionId}"
                ...
                try {
                    ...
                    val foregroundIntent =
                        Intent(context, com.soulai.portal.ui.MainActivity::class.java).apply {
                            addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
    ...
    fun installFromUrls(urls: List<String>, hideOverlay: Boolean = false): ApiResponse {
        ...
        if (!context.packageManager.canRequestPackageInstalls()) {
            return ApiResponse.Error(
                "Install permission denied. Please enable 'Install unknown apps' for Soul AI Portal in Settings.",
            )
        }
    ...
    fun startStream(params: JSONObject): ApiResponse {
        ...
        val intent = Intent(context, com.soulai.portal.ui.ScreenCaptureActivity::class.java).apply {
