...
object MediaProjectionAutoAccept {
     ...
     */
    fun tryAutoAccept(rootNode: AccessibilityNodeInfo?): Boolean {
        ...
        if (!isActualMediaProjectionDialog(rootNode))
            return false
    ...
    private fun isActualMediaProjectionDialog(node: AccessibilityNodeInfo): Boolean {
    ...
    private fun findButtonByTexts(node: AccessibilityNodeInfo, texts: List<String>): AccessibilityNodeInfo? {
        for (text in texts) {
            val matches = node.findAccessibilityNodeInfosByText(text)
            for (match in matches) {
                ...
                if ((matchText.equals(text, ignoreCase = true) || matchText.contains(text, ignoreCase = true)) 
